     ...
     <div class="card-body">
         ...
         <form method="POST" action="{{ url('/stok/'.$stok->stok_id) }}" class="form-horizontal">
             ...
             <div class="form-group row">
                 ...
                 <div class="col-10">
                     <input type="datetime-local" class="form-control" id="stok_tanggal" name="stok_tanggal" value="{{ old('stok_tanggal', \Carbon\Carbon::parse($stok->stok_tanggal)->format('Y-m-d\TH:i')) }}" required>
                     @error('stok_tanggal')
                         <small class="form-text text-danger">{{ $message }}</small>
                     @enderror
                 </div>
